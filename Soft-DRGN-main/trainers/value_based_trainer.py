...
#0表示水平镜面对称，1表示垂直镜面对称，2表示顺时针90°，3表示顺时针180°，4表示顺时针270°
def trans_obs(obs, trans_type,index_map_list): #改变obs的函数
    ...
    # for i in range(527):
    #         map1[i] = temp_map1[int(index_map_list[i])]
    #         map2[i] = temp_map2[int(index_map_list[i])]
    #         map3[i] = temp_map3[int(index_map_list[i])]
    # import pickle
    # print(map1)
    # f=open("filename1",'wb')
    # pickle.dump(map1,f)
        ...
        for index,trans_type in enumerate(hparams["data_trans_list"]):
            ...
            for sample in sample_list:
                ...
                for i in range(hparams["env_num_agent"]):
                    ...
                    # temp_sample["adj"][i] = trans_adj(temp_sample["adj"][i], trans_type)
                    ...
                    # temp_sample["next_adj"][i] = trans_adj(temp_sample["next_adj"][i], trans_type)
